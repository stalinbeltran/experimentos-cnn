...
import datos
from modelo import BRAZOS, CELDAS, LIMITES, VENTANA, construir, simetria
...
# ⚠ AQUI SE DIBUJAN LINEAS, NO CRUCES, y no es estetica: la salida de esta red es
# UNA ALTURA. Una cruz sugiere que la `x` significa algo, y no significa nada --
# es justo el malentendido que el cambio de `esq-cq` a `lim-h` quiere quitar.
COLOR_PRED = (230, 0, 0)                                # la altura predicha
COLOR_REAL = {"sup": (0, 150, 0), "inf": (0, 90, 200)}  # la verdadera, por lado
VERDE = (0, 190, 0)
...
def figura(brazo: str, d10: dict, etiqueta: str) -> Path:
    ...
    with torch.no_grad():
        (logit, py), mapa = red(torch.from_numpy(V).unsqueeze(1))
        pred_x = red.diagnostico(mapa).numpy()   # DIAGNOSTICO, no salida
    p_existe, pred_y = torch.sigmoid(logit).numpy(), py.numpy()
    # La etiqueta unica, por la MISMA puerta que usa el entrenamiento.
    e_real, y_real = datos.objetivo(d10)
    ...
    ok = tot = 0
    por_lado = {c: [0, 0] for c in LIMITES}
    for i in range(len(V)):
        if e_real[i] != 1:
            continue
        e = abs(float(pred_y[i] - y_real[i]))        # SOLO la altura
        ok += int(e <= 2); tot += 1
        for c in LIMITES:
            if d10[f"existe_{c}"][i] == 1:
                por_lado[c][0] += int(e <= 2); por_lado[c][1] += 1
    resumen = f"{ok}/{tot}" + (
        "  (" + " · ".join(f"{c} {por_lado[c][0]}/{por_lado[c][1]}"
                           for c in LIMITES if por_lado[c][1]) + ")")
    ...
    dr.text((sep, 10), f"{brazo} · estructura `{red.estructura}` · kernel {k}x{k} · "
                       ...
                       f"acierta a <=2 px de ALTURA: {resumen}",
    ...
    dr.text((sep, 32), f"arriba: ventana 32x32 (x8) · abajo: mapa del kernel en las MISMAS "
                       ...
                       f"· kernel {100*s_vert:.0f}% simetrico VERTICAL ({100*s_180:.0f}% rot180)",
            fill=(90, 90, 90), font=f)
    dr.text((sep, 50), "LINEA VERDE: el limite verdadero si es el SUPERIOR · AZUL: si es el "
                       "INFERIOR · LINEA ROJA: la UNICA altura predicha — son lineas y no "
                       "cruces porque la salida es una ALTURA: la `x` no significa nada aqui",
            fill=(90, 90, 90), font=f)
    ...
    for i in range(len(V)):
        ...
        for oy, r, w in ((y0, 14, 3), (ym, 10, 2)):
            ...
            for c in LIMITES:                           # el verdadero, si existe
                if d10[f"existe_{c}"][i] == 1:
                    yy = oy + float(d10[f"y_{c}"][i]) * ESCALA
                    cap.line([x0, yy, x0 + LADO - 1, yy], fill=COLOR_REAL[c], width=w + 1)
            # UNA sola altura predicha, de lado a lado
            yp = oy + float(pred_y[i]) * ESCALA
            cap.line([x0, yp, x0 + LADO - 1, yp], fill=COLOR_PRED, width=w)
            dr.rectangle([x0, oy, x0 + LADO - 1, oy + LADO - 1], outline=(120, 120, 120))

        dr.text((x0, ym + LADO + 5), f"{i+1}. {d10['clase'][i]}", fill=(20, 20, 20), font=f)
        lin = ym + LADO + 23
        cual = next((c for c in LIMITES if d10[f"existe_{c}"][i] == 1), None)
        real = e_real[i] == 1
        # ⚠ CORTO a proposito: la columna mide LADO px y un pie mas largo se
        # solapa con el de la ventana siguiente, que es como se pierde legibilidad
        # justo en la figura que existe para mirar.
        txt = (f"{cual if cual else 'no'} · p {float(p_existe[i]):.2f}")
        color = (70, 70, 70)
        if real:
            e = abs(float(pred_y[i] - y_real[i]))
            # El umbral de 2 px es el de la metrica principal del criterio,
            # para que la figura y el criterio digan lo mismo.
            txt += f" · dy {e:.1f} {'ok' if e <= 2 else 'FALLA'}"
...
def main() -> int:
    ...
    print(f"{len(d10['clase'])} muestras · {int(d10['existe_sup'].sum())} con limite "
          f"SUPERIOR · {int(d10['existe_inf'].sum())} con INFERIOR")
